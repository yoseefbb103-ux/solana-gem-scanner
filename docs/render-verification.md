...
## سياسة Telegram الجديدة — 2026-08-28

أعيد تصميم طبقة Telegram لتكون قناة انتقائية للجواهر فقط. لا تُرسل رسائل `EARLY WATCH` ولا رسائل `decision_flip` أو `liquidity_pull`. تمر الرسالة النهائية ببوابة موحدة تشترط مستوى `strong` أو `confirmed`، قرار «مراقبة»، فحص أمان مارّ، سيولة لا تقل عن 10,000 دولار، حجم ساعة لا يقل عن 5,000 دولار، فرصة 62 أو أكثر، مخاطرة 35 أو أقل، سعراً صالحاً، وعدم وجود سحب سيولة أو ناشر مرتبط بسجل رَقّ.

أصبح قالب الرسالة عربياً ومختصراً: اسم ورمز العملة، مستوى الإشارة، الفرصة والمخاطرة، السعر والسيولة والحجم والمعاملات والعمر، عاملان داعمان فقط، وتحذير مهم واحد عند وجوده، مع رابط DEX والعقد والتحذير المالي. عند توفر رابط صورة HTTPS يستخدم `sendPhoto`، وإذا رفض Telegram الصورة يعود إلى `sendMessage` دون فقدان التنبيه. هذا الفرز لا يتنبأ بعائد ولا يحوّل النظام إلى أداة تداول؛ التطبيق للقراءة فقط.

تحقق محلياً من `pnpm test` بنتيجة 45 اختباراً ناجحاً واختبار واحد متروك عمداً، ومن `pnpm check` و`pnpm build` بعد التعديل. يبقى نشر Render الخارجي معلقاً حتى وصول commit هذا التعديل إلى GitHub ثم اكتمال Auto-Deploy في Web وWorker Ohio.
